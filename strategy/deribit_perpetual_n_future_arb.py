...
class FutureArbitrage(ServiceBase):
    ...
    async def find_quotes_gap(self):
        try:
            ...
            if all((min(future.bid-perpetual.bid, future.ask-perpetual.ask) >= N_TX_ENTRY_PRICE_GAP/100*future.index_price,
            ...
            elif all((min(perpetual.bid-future.bid, perpetual.ask-future.ask) >= N_TX_ENTRY_PRICE_GAP/100*future.index_price,
                      premium <= - N_TX_ENTRY_GAP[pos_idx])):
                if not f_limit_order.if_placed:
            ...
            elif premium <= N_TX_EXIT_GAP and future_size < 0 and perpetual_size > 0:
                if not f_limit_order.if_placed:
